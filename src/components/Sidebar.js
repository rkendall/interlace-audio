...
class SideBar extends Component {
  constructor(props) {
    ...
    this.menu = createRef()
    const initialInd = compositionTitles.findIndex(
      ({ name }) => name === initialSelectedValue,
    )
    const { hasPoetry } = compositionTitles[initialInd]
    this.state = {
      selectedInd: initialInd,
  ...
  componentDidUpdate(prevProps) {
    ...
    this.setState(({ isOpen }) =>
      sidebarOpen !== isOpen ? { isOpen: sidebarOpen } : null,
    )
  }
  ...
  render() {
    ...
    const poetryAvailableTooltip = poetrySelected
      ? 'Turn Word Art off'
      : 'Add Word Art to the piece'
    const poetryTooltip = hasPoetry
      ? poetryAvailableTooltip
      : 'Sorry, no Word Art for this piece'
    return (
      <Fragment>
        {!isInstallation && (
          <div
        ...
        <div className="sidebar">
          ...
          <button
            className="up arrow box"
            onClick={this.selectNextOrPrevious.bind(null, 'previous')}
          >
            <KeyboardArrowUp />
          </button>
          <div className="menuWrapper">
            <div id="menu" className="box menu" ref={this.menu}>
              {compositionTitles.map(({ name, title }, ind) => (
                <div
                  className={classNames('menuOption', {
                    selected: ind === selectedInd,
                  })}
                  onMouseDown={this.onChange.bind(null, ind)}
                  key={name}
                >
                  <Element name={name} key={name}>
                    <div className="optionText">
                      <div className="time">
                        {this.getTimeForComposition(name)}
                      </div>
          ...
          <button
            className="down arrow box"
            onClick={this.selectNextOrPrevious.bind(null, 'next')}
          >
            <KeyboardArrowDown />
          </button>
          {!isInstallation && (
            <>
              <div className="box controls">
                <div className="instructions">
                  Click and hold square to start/stop looping
                </div>
                <div className="selectOptions">
                  <div className="selectOption">
                    ...
                    <label
                      htmlFor="smartLooping"
                      data-tip="Vary the music on each loop"
                      data-for="smartLoopingTip"
                    >
                      Smart Looping
                    </label>
                  </div>
                ...
                <div className="buttonBar">
                  <Button onClick={onStopLooping}>Stop All Looping</Button>
                </div>
              </div>
              }
              <div className="box controls">
                <div className="selectOptions">
                  ...
                  <div className="selectOption">
                    ...
                    <label
                      htmlFor="poetry"
                      className={hasPoetry ? '' : 'selectionDisabled'}
                      data-tip={poetryTooltip}
                      data-for="poetryTip"
                    >
                ...
                <div className="buttonBar">
                  <Button onClick={onShowAllSquares}>
                    Show All Instruments
                  </Button>
                  <Button onClick={this.onToggleMessage}>
